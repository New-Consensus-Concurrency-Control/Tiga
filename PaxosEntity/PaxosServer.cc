...
DEFINE_int32(shardNum, 3, "The number of shards");
DEFINE_int32(replicaNum, 3, "The number of replicas");
DEFINE_int32(shardId, 0, "ShardId");
DEFINE_int32(replicaId, 0, "ReplicaId");
...
int main(int argc, char** argv) {
   ...
   PaxosReplica* replica =
       new PaxosReplica(FLAGS_serverName, config, FLAGS_shardId,
                        FLAGS_replicaId, FLAGS_shardNum, FLAGS_replicaNum);
   LOG(INFO) << "Replica Created";

   // Replica
   PaxosServiceImpl* replicaService = new PaxosServiceImpl(replica);
