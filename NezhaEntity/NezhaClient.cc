...
int main(int argc, char** argv) {
   ...
   nezha::NezhaBridgeProxy* proxy = new nezha::NezhaBridgeProxy(rpcClient);
   ...
   while (true) {
      NezhaRecordMessage msg;
      uint32_t reqId = nextReqId;
      msg.cmdId_ = reqId;
      msg.ssidHigh_ = random();
      msg.ssidLow_ = random();
      msg.ssidNew_ = random();
      msg.txnKey_ = CONCAT_UINT32(id, reqId);
      int a = random() % 10 + 1;
      msg.keys_.push_back(a);
      msg.keys_.push_back((a + 1) % 10 + 1);
      msg.keys_.push_back((a + 3) % 10 + 1);
      ...
      std::function<void(Future*)> cb = [reqId](Future* fu) {
         ...
         NezhaAck ack;
         fu->get_reply() >> ack;
         LOG(INFO) << "ack " << HIGH_32BIT(ack.token_) << ":"
                   << LOW_32BIT(ack.token_);
         ackNum++;
      };
      fuattr.callback = cb;
      sendTimes[msg.cmdId_] = GetMicrosecondTimestamp();
      Future::safe_release(proxy->async_Persist(msg, fuattr));
