...
ResultSet Txn::query_lt(Table *tbl, const MultiBlob &mb,
                        symbol_t order /* =? */) {
   return query_lt(tbl, SortedMultiKey(mb, tbl->schema()), order);
}

ResultSet Txn::query_lt(Table *tbl, const MultiBlob &mb, bool retrieve,
                        int64_t pid, symbol_t order /* =? */) {
   switch (rtti()) {
      // case symbol_t::TXN_2PL:
      //    return ((Txn2PL *)this)
      //        ->query_lt(tbl, SortedMultiKey(mb, tbl->schema()), retrieve,
      //        pid,
      //                   order);
      default:
         return query_lt(tbl, SortedMultiKey(mb, tbl->schema()), order);
...
ResultSet Txn::query_gt(Table *tbl, const MultiBlob &mb,
                        symbol_t order /* =? */) {
   return query_gt(tbl, SortedMultiKey(mb, tbl->schema()), order);
}

ResultSet Txn::query_gt(Table *tbl, const MultiBlob &mb, bool retrieve,
                        int64_t pid, symbol_t order /* =? */) {
   switch (rtti()) {
      // case symbol_t::TXN_2PL:
      //    return ((Txn2PL *)this)
      //        ->query_gt(tbl, SortedMultiKey(mb, tbl->schema()), retrieve,
      //        pid,
      //                   order);
      default:
         return query_gt(tbl, SortedMultiKey(mb, tbl->schema()), order);
...
ResultSet Txn::query_in(Table *tbl, const MultiBlob &low, const MultiBlob &high,
                        symbol_t order /* =? */) {
...
ResultSet Txn::query_in(Table *tbl, const MultiBlob &low, const MultiBlob &high,
                        bool retrieve, int64_t pid, symbol_t order /* =? */) {
   switch (rtti()) {
      // case symbol_t::TXN_2PL:
      //    return ((Txn2PL *)this)
      //        ->query_in(tbl, SortedMultiKey(low, tbl->schema()),
      //                   SortedMultiKey(high, tbl->schema()), retrieve, pid,
      //                   order);
      default:
         return query_in(tbl, SortedMultiKey(low, tbl->schema()),
                         SortedMultiKey(high, tbl->schema()), order);
