...
Txn2PL::Txn2PL(const TxnMgr *mgr, txn_id_t txnid)
    : Txn(mgr, txnid), outcome_(symbol_t::NONE) {}
...
ResultSet Txn2PL::query(Table *tbl, const MultiBlob &mb, bool retrieve,
                        int64_t pid) {
...
void Txn2PL::marshal_stage(std::string &str) {
   ...
   for (auto &it : reads_) {
      ...
      for (int i = 0; i < count; i++) {
         ...
         memcpy((void *)(str.data() + len), (void *)(&(mb[i].len)),
                sizeof(mb[i].len));
         len += sizeof(mb[i].len);
      ...
      memcpy((void *)(str.data() + len), (void *)(&it.second),
             sizeof(it.second));
   ...
   memcpy((void *)(str.data() + len), (void *)&num_inserts,
          sizeof(num_inserts));
   ...
   memcpy((void *)(str.data() + len), (void *)&num_updates,
          sizeof(num_updates));
   ...
   for (auto &it : updates_) {
      ...
      memcpy((void *)(str.data() + len), (void *)(&it.second.first),
   ...
   memcpy((void *)(str.data() + len), (void *)&num_removes,
          sizeof(num_removes));
...
bool Txn2PL::commit() {
   ...
   for (auto &it : removes_) {
      ...
      it.table->remove(
          it.row, false /* DO NOT free the row until it's erased from tbl */);
      rows_to_remove.push_back(it.row);
   }
   outcome_ = symbol_t::TXN_COMMIT;
   release_resource();
   for (auto &it : rows_to_remove) it->release();
   return true;
}
...
ResultSet Txn2PL::do_query_lt(Table *tbl, const SortedMultiKey &smk,
                              symbol_t order /* =? */) {
...
ResultSet Txn2PL::do_query_gt(Table *tbl, const SortedMultiKey &smk,
                              symbol_t order /* =? */) {
   verify(order == symbol_t::ORD_ASC || order == symbol_t::ORD_DESC ||
          order == symbol_t::ORD_ANY);
...
ResultSet Txn2PL::do_query_in(Table *tbl, const SortedMultiKey &low,
