...
TxnOCC::TxnOCC(const TxnMgr *mgr, txn_id_t txnid,
               const std::vector<std::string> &table_names)
    : Txn2PL(mgr, txnid), verified_(false) {
...
TxnOCC::~TxnOCC() { release_resource(); }
   ...
   for (auto &pair : ver_check_read_) {
      ...
      if (it != ver_check_write_.end()) {
         ...
         if (ver_read < ver_write) {
            ...
            uint64_t iii = id();
            Log_debug("Txnid %lx, Table Name %s, col_id %d", iii,
                      r->get_table()->Name().c_str(), c);
...
bool TxnOCC::version_check(
    const std::unordered_map<row_column_pair, version_t, row_column_pair::hash>
        &ver_info) {
...
bool TxnOCC::commit_prepare() { verify(0); }

void TxnOCC::commit_confirm() { verify(0); }
...
bool TxnOCC::read_column(Row *row, column_id_t col_id, Value *value) {
   ...
   if (row->rtti() == symbol_t::ROW_VERSIONED) {
      VersionedRow *v_row = (VersionedRow *)row;
      insert_into_map(ver_check_read_, row_column_pair(v_row, col_id),
                      v_row->get_column_ver(col_id));
      // increase row reference count because later we are going to check its
      // version
   ...
   if (it == it_pair.second) {
      if (row->rtti() == symbol_t::ROW_VERSIONED) {
         ...
         for (size_t col_id = 0; col_id < v_row->schema()->columns_count();
            ...
            insert_into_map(ver_check_write_, row_column_pair(v_row, col_id),
                            v_row->get_column_ver(col_id));
            // increase row reference count because later we are going to check
            // its version
            incr_row_refcount(row);
         }
