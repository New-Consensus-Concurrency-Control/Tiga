...
class ResultSet : public Enumerator<Row *> {
  ...
  public:
   ...
   bool has_next() { return rows_->has_next(); }
   Row *next() { return const_cast<Row *>(rows_->next()); }
};
...
class Txn : public NoCopy {
  ...
  public:
   ...
   txn_id_t id() const { return txnid_; }
   ...
   virtual bool write_column(Row *row, column_id_t col_id,
                             const Value &value) = 0;
   virtual bool insert_row(Table *tbl, Row *row) = 0;
   virtual bool remove_row(Table *tbl, Row *row) = 0;

   bool read_columns(Row *row, const std::vector<column_id_t> &col_ids,
   ...
   bool write_columns(Row *row, const std::vector<column_id_t> &col_ids,
   ...
   ResultSet query(Table *tbl, const Value &kv) {
      Log_info("tableName=%s rtti=%d", tbl->name_.c_str(), tbl->rtti());
   ...
   virtual ResultSet query(Table *tbl, const MultiBlob &mb, bool retrieve,
                           int64_t pid) {
      verify(rtti() != symbol_t::TXN_2PL);
      return query(tbl, mb);
   }

   ResultSet query_lt(Table *tbl, const Value &kv,
                      symbol_t order = symbol_t::ORD_ASC) {
      return query_lt(tbl, kv.get_blob(), order);
   }
   ResultSet query_lt(Table *tbl, const Value &kv, bool retrieve, int64_t pid,
                      symbol_t order = symbol_t::ORD_ASC) {
      return query_lt(tbl, kv.get_blob(), retrieve, pid, order);
   }
   ResultSet query_lt(Table *tbl, const MultiBlob &mb,
                      symbol_t order = symbol_t::ORD_ASC);
   ResultSet query_lt(Table *tbl, const MultiBlob &mb, bool retrieve,
                      int64_t pid, symbol_t order = symbol_t::ORD_ASC);
   virtual ResultSet query_lt(Table *tbl, const SortedMultiKey &smk,
                              symbol_t order = symbol_t::ORD_ASC) = 0;

   ResultSet query_gt(Table *tbl, const Value &kv,
                      symbol_t order = symbol_t::ORD_ASC) {
      return query_gt(tbl, kv.get_blob(), order);
   }
   ResultSet query_gt(Table *tbl, const Value &kv, bool retrieve, int64_t pid,
                      symbol_t order = symbol_t::ORD_ASC) {
      return query_gt(tbl, kv.get_blob(), retrieve, pid, order);
   }
   ResultSet query_gt(Table *tbl, const MultiBlob &mb,
                      symbol_t order = symbol_t::ORD_ASC);
   ResultSet query_gt(Table *tbl, const MultiBlob &mb, bool retrieve,
                      int64_t pid, symbol_t order = symbol_t::ORD_ASC);
   virtual ResultSet query_gt(Table *tbl, const SortedMultiKey &smk,
                              symbol_t order = symbol_t::ORD_ASC) = 0;

   ResultSet query_in(Table *tbl, const Value &low, const Value &high,
                      symbol_t order = symbol_t::ORD_ASC) {
   ...
   ResultSet query_in(Table *tbl, const Value &low, const Value &high,
                      ...
                      symbol_t order = symbol_t::ORD_ASC) {
      return query_in(tbl, low.get_blob(), high.get_blob(), retrieve, pid,
                      order);
   }
   virtual ResultSet query_in(Table *tbl, const MultiBlob &low,
                              const MultiBlob &high,
                              symbol_t order = symbol_t::ORD_ASC);
   ResultSet query_in(Table *tbl, const MultiBlob &low, const MultiBlob &high,
                      bool retrieve, int64_t pid,
                      symbol_t order = symbol_t::ORD_ASC);

   virtual ResultSet query_in(Table *tbl, const SortedMultiKey &low,
                              const SortedMultiKey &high,
                              symbol_t order = symbol_t::ORD_ASC) = 0;

   virtual ResultSet all(Table *tbl, symbol_t order = symbol_t::ORD_ANY) = 0;
   virtual ResultSet all(Table *tbl, bool retrieve, int64_t pid,
...
//  static std::map<const Table *, uint64_t> wl_conflict_count_;
   ...
   virtual ~TxnMgr() {
      ...
      for (auto it : tables_)
         Log::info(
             "CONFLICT COUNT: Table: %10s,\tversion check: %5llu, read: "
             "%5llu\twrite: %5llu",
             it.first.c_str(), vc_conflict_count_[it.second],
             rl_conflict_count_[it.second], wl_conflict_count_[it.second]);
#endif
   }
   virtual symbol_t rtti() const = 0;
...
struct column_lock_t {
   ...
   column_lock_t(Row *_row, column_id_t _column_id, rrr::ALock::type_t _type)
...
class KeyOnlySearchRow : public Row {
  ...
  public:
   ...
   virtual MultiBlob get_key() const { return *mb_; }
};
...
class MergedCursor : public NoCopy, public Enumerator<const Row *> {
  ...
  public:
   MergedCursor(
       Table *tbl, Enumerator<const Row *> *cursor,
       ...
       : tbl_(tbl),
         ...
         inserts_next_(inserts_begin),
         inserts_end_(inserts_end),
         removes_(removes),
         cached_(false),
         cached_next_(nullptr),
         next_candidate_(nullptr) {}

   MergedCursor(
       Table *tbl, Enumerator<const Row *> *cursor,
       const std::multiset<table_row_pair>::const_reverse_iterator
           &inserts_rbegin,
       const std::multiset<table_row_pair>::const_reverse_iterator
           &inserts_rend,
       const std::unordered_set<table_row_pair, table_row_pair::hash> &removes)
       : tbl_(tbl),
         cursor_(cursor),
         reverse_order_(true),
         r_inserts_next_(inserts_rbegin),
         r_inserts_end_(inserts_rend),
         removes_(removes),
         cached_(false),
         cached_next_(nullptr),
         next_candidate_(nullptr) {}

   ~MergedCursor() { delete cursor_; }
