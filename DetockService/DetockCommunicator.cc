#include "DetockCommunicator.h"

DetockCommunicator::DetockCommunicator(const uint32_t id,
...
void DetockCommunicator::Connect() {
   ...
   for (uint32_t sid = 0; sid < shardNum_; sid++) {
      for (uint32_t rid = 0; rid < replicaNum_; rid++) {
         ...
         proxies_[sid][rid] = new DetockProxy(cli);
      }
   }
   LOG(INFO) << "All Connected";
}

DetockProxy* DetockCommunicator::ProxyAt(const uint32_t shardId,
                                         const uint32_t replicaId) {
   return proxies_[shardId][replicaId];
}

DetockCommunicator::~DetockCommunicator() {}