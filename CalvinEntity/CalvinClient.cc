...
DEFINE_int32(designateShardId, 0, "The desginate sequencer's shardId");
DEFINE_int32(designateReplicaId, 0, "The desginate sequencer's replicaId");
...
uint32_t shardNum;
uint32_t replicaNum;
int32_t desginatedShardId;
int32_t designatedReplicaId;
...
std::vector<CalvinCoordinator*> createdCoordinators;
std::vector<CalvinCoordinator*> freeCoordinators;
...
void DispatchRequest(CalvinCoordinator* coo, uint32_t clientId);
void RequestDone(CalvinCoordinator* coo, const ClientReply& rep);

CalvinCoordinator* FindOrCreateCoordinator() {
   CalvinCoordinator* coo = NULL;
   ...
   if (freeCoordinators.size() > 0) {
      ...
      coo = new CalvinCoordinator(
          createdCoordinators.size() + 1 + coordinatorIdOffset, shardNum,
          replicaNum, desginatedShardId, designatedReplicaId, comm, config);
...
void ClientWorker(uint32_t clientId) {
   ...
   if (isOpenLoop) {
      ...
      CalvinCoordinator* coo = FindOrCreateCoordinator();
      DispatchRequest(coo, clientId);
   }
}
...
int main(int argc, char* argv[]) {
   ...
   desginatedShardId = FLAGS_designateShardId;
   designatedReplicaId = FLAGS_designateReplicaId;
   ...
   comm = new CalvinCommunicator(myIdx, config);
   LOG(INFO) << "Communicator idx= " << myIdx;
   comm->Connect();
   LOG(INFO) << "Connected";
   LOG(INFO) << "CoordinatorId=" << myIdx + 1;
   LOG(INFO) << "desginatedShardId=" << desginatedShardId << "\t"
             << "designatedReplicaId=" << designatedReplicaId;
