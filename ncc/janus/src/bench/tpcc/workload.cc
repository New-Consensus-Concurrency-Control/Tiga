...
TpccWorkload::TpccWorkload(Config *config) : Workload(config) {
...
void TpccWorkload::GetNewOrderTxnReq(TxRequest *req, uint32_t cid) const {
   ...
   for (int i = 0; i < ol_cnt; i++) {
      // req->input_[4 + 3 * i] = Value((i32)RandomGenerator::nu_rand(8191, 0,
...
void TpccWorkload::get_tpcc_payment_txn_req(TxRequest *req,
                                            uint32_t cid) const {
   ...
   Value d_id((i32)(cid / tpcc_para_.n_w_id_) % tpcc_para_.n_d_id_);
   if (true ||
       RandomGenerator::percentage_true(60)) {  // XXX query by last name 60%
      req->input_[TPCC_VAR_C_LAST] = Value(
   ...
   //  Log_info("input size=%lu has cid?=%d", req->input_.size(),
   //           req->input_.count(TPCC_VAR_C_ID));
}

void TpccWorkload::get_tpcc_stock_level_txn_req(TxRequest *req,
                                                uint32_t cid) const {
...
void TpccWorkload::get_tpcc_delivery_txn_req(TxRequest *req,
                                             uint32_t cid) const {
...
void TpccWorkload::get_tpcc_order_status_txn_req(TxRequest *req,
                                                 uint32_t cid) const {
   req->tx_type_ = TPCC_ORDER_STATUS;
   if (true || RandomGenerator::percentage_true(60)) {  // XXX 60% by c_last
      req->input_[TPCC_VAR_C_LAST] = Value(
          RandomGenerator::int2str_n(RandomGenerator::nu_rand(255, 0, 999), 3));
   } else {
      req->input_[TPCC_VAR_C_ID] =
...
void TpccWorkload::GetTxRequest(TxRequest *req, uint32_t cid) {
   ...
   if (txn_weight_.size() != 5) {
      ...
      // Log_info("2-shot order status txn");
      // get_tpcc_order_status_txn_req(req, cid);
      // Log_info("2-shot payment txn");
