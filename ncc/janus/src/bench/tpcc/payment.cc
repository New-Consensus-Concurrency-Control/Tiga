...
void TpccProcedure::PaymentInit(TxRequest& req) {
   ...
   if (ws_.count(TPCC_VAR_C_LAST) > 0) {
      // GJK: Log_info("payment transaction lookup by customer name");
   ...
   // GJK
   // for(auto& kv: status_) {
   //   Log_info("First Checkpiece id=%d status =%d", kv.first, kv.second);
   // }
}
...
void TpccWorkload::RegPayment() {
   // Log_info("Register Payment Procedure");
   // piece 0, Ri & W warehouse
   set_op_type(TPCC_PAYMENT, TPCC_PAYMENT_0, READ_REQ);
   RegP(
       TPCC_PAYMENT, TPCC_PAYMENT_0,
       {TPCC_VAR_W_ID, TPCC_VAR_D_ID, TPCC_VAR_H_AMOUNT, TPCC_VAR_C_W_ID,
        TPCC_VAR_C_D_ID, TPCC_VAR_H_KEY},  // i
       ...
       DF_NO, PROC {
          ...
          // Log_info("TPCC_PAYMENT, piece: %d has cid?=%d", TPCC_PAYMENT_0,
          //          cmd.input.count(TPCC_VAR_C_ID));
          i32 oi = 0;
          mdb::Row* row_warehouse =
              tx.Query(tx.GetTable(TPCC_TB_WAREHOUSE),
                       cmd.input[TPCC_VAR_W_ID].get_blob(), ROW_WAREHOUSE);
          // R warehouse
          output[TPCC_VAR_W_NAME] = Value("");
          tx.ReadColumn(row_warehouse, TPCC_COL_WAREHOUSE_W_NAME,
                        &output[TPCC_VAR_W_NAME], TXN_BYPASS);
          tx.ReadColumn(row_warehouse, TPCC_COL_WAREHOUSE_W_STREET_1,
                        &output[TPCC_VAR_W_STREET_1], TXN_BYPASS);
          tx.ReadColumn(row_warehouse, TPCC_COL_WAREHOUSE_W_STREET_2,
                        &output[TPCC_VAR_W_STREET_2], TXN_BYPASS);
          tx.ReadColumn(row_warehouse, TPCC_COL_WAREHOUSE_W_CITY,
                        &output[TPCC_VAR_W_CITY], TXN_BYPASS);
          tx.ReadColumn(row_warehouse, TPCC_COL_WAREHOUSE_W_STATE,
                        &output[TPCC_VAR_W_STATE], TXN_BYPASS);
          tx.ReadColumn(row_warehouse, TPCC_COL_WAREHOUSE_W_ZIP,
                        &output[TPCC_VAR_W_ZIP], TXN_BYPASS);
          *res = SUCCESS;
       });
   ...
   RegP(
       TPCC_PAYMENT, TPCC_PAYMENT_1, {TPCC_VAR_W_ID, TPCC_VAR_D_ID},  // i
       {TPCC_VAR_D_NAME},                                             // o
       {},                                                            // c TODO
       {TPCC_TB_DISTRICT, {TPCC_VAR_W_ID}}, DF_NO, PROC {
          verify(cmd.input.size() >= 2);
          // Log_info("TPCC_PAYMENT, piece: %d has cid?=%d", TPCC_PAYMENT_1,
          //          cmd.input.count(TPCC_VAR_C_ID));
          Value buf;
          mdb::MultiBlob mb(2);
          ...
          // R district
          tx.ReadColumn(row_district, TPCC_COL_DISTRICT_D_NAME,
                        &output[TPCC_VAR_D_NAME], TXN_BYPASS);
          tx.ReadColumn(row_district, TPCC_COL_DISTRICT_D_STREET_1,
                        &output[TPCC_VAR_D_STREET_1], TXN_BYPASS);
          tx.ReadColumn(row_district, TPCC_COL_DISTRICT_D_STREET_2,
                        &output[TPCC_VAR_D_STREET_2], TXN_BYPASS);
          tx.ReadColumn(row_district, TPCC_COL_DISTRICT_D_CITY,
                        &output[TPCC_VAR_D_CITY], TXN_BYPASS);
          tx.ReadColumn(row_district, TPCC_COL_DISTRICT_D_STATE,
                        &output[TPCC_VAR_D_STATE], TXN_BYPASS);
          tx.ReadColumn(row_district, TPCC_COL_DISTRICT_D_ZIP,
                        &output[TPCC_VAR_D_ZIP], TXN_BYPASS);
   ...
   RegP(
       ...
       DF_REAL, PROC {
          ...
          // Log_info("TPCC_PAYMENT, piece: %d has cid?=%d", TPCC_PAYMENT_2,
          //          cmd.input.count(TPCC_VAR_C_ID));
          ...
          tx.ReadColumn(row_temp, TPCC_COL_DISTRICT_D_YTD, &buf_temp,
                        TXN_BYPASS);
          // W district
          Value buf(0.0);
          ...
          tx.WriteColumn(
              row_temp, TPCC_COL_DISTRICT_D_YTD,
   ...
   RegP(
       ...
       DF_REAL, PROC {
          verify(cmd.input.size() >= 6);
          // Log_info("TPCC_PAYMENT, piece: %d has cid?=%d", TPCC_PAYMENT_5,
          //          cmd.input.count(TPCC_VAR_C_ID));
   ...
   RegP(
       ...
       DF_NO, PROC {
          verify(cmd.input.size() >= 3);
          // Log_info("TPCC_PAYMENT, piece: %d has cid?=%d", TPCC_PAYMENT_3,
          //          cmd.input.count(TPCC_VAR_C_ID));
          ...
          c_last_id_t key_low(cmd.input[TPCC_VAR_C_LAST].get_str(), mbl,
                              &C_LAST_SCHEMA);
          c_last_id_t key_high(cmd.input[TPCC_VAR_C_LAST].get_str(), mbh,
                               &C_LAST_SCHEMA);
          ...
          Log_debug("w_id: %d, d_id: %d, c_last: %s, num customer: %d",
                    ...
                    cmd.input[TPCC_VAR_C_LAST].get_str().c_str(), n_c);
          verify(mid_set);
   ...
   RegP(
       ...
       DF_REAL, PROC {
          verify(cmd.input.size() >= 6);
          // Log_info("TPCC_PAYMENT, piece: %d has cid?=%d", TPCC_PAYMENT_4,
          //          cmd.input.count(TPCC_VAR_C_ID));
          mdb::Row* r = NULL;
          mdb::MultiBlob mb(3);
          ...
          vector<Value> buf({Value(""), Value(""), Value(""), Value(""),
          ...
          tx.ReadColumn(r, TPCC_COL_CUSTOMER_C_CREDIT_LIM, &buf[11],
                        TXN_BYPASS);
          ...
          tx.ReadColumn(r, TPCC_COL_CUSTOMER_C_YTD_PAYMENT, &buf[14],
                        TXN_DEFERRED);
