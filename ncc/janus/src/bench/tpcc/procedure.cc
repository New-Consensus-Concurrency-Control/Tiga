...
bool TpccProcedure::CheckReady() {
   ...
   for (auto& pair : status_) {
      ...
      bool all_found = true;
      // GJK Log_info("to check piece =%d input size=%lu", pi,
      // input_vars.size()); for (auto &var : sharding_vars) { // is it a
      // mistake?
      for (auto& var : input_vars) {
         // Log_info("piece=%d Why Sharding? var = %d", pi, var);
         if (ws_.count(var) == 0) {
            ...
            if (ws.keys_.size() == 0) ws.keys_ = var_set;
            verify(ws_[var].get_kind() != 0);
#endif
         }
      }
      // Log_info(
      //     "complete check piece =%d input size=%lu (all_found && "
      //     "status==WAITING)=%d",
      //     pi, input_vars.size(), (all_found && status == WAITING));
...
bool TpccProcedure::HandleOutput(int pi, int res,
   ...
   // below is for debug
   if (type_ == TPCC_PAYMENT || type_ == TPCC_ORDER_STATUS ||
       type_ == TPCC_DELIVERY || type_ == TPCC_NEW_ORDER || 0) {
      // for debug
...
int TpccProcedure::GetNPieceAll() {
   if (type_ == TPCC_STOCK_LEVEL) {
      verify(ws_.count(TPCC_VAR_OL_AMOUNT) > 0 ==
             ws_.count(TPCC_VAR_N_PIECE_ALL) > 0);
