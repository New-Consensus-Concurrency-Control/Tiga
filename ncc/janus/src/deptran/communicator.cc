...
Communicator::Communicator(PollMgr* poll_mgr) {
   ...
   for (auto& par_id : partitions) {
      ...
      for (auto& si : site_infos) {
         Log_info("Connect to site %s", si.GetHostAddr().c_str());
         auto result =
             ConnectToSite(si, std::chrono::milliseconds(CONNECT_TIMEOUT_MS));
         Log_info("Connected to site %s", si.GetHostAddr().c_str());
...
void Communicator::ConnectClientLeaders() {
   ...
   if (config->forwarding_enabled_) {
      ...
      for (Config::SiteInfo leader_site_info : client_leaders) {
         ...
         auto result = ConnectToClientSite(
             leader_site_info, std::chrono::milliseconds(CONNECT_TIMEOUT_MS));
         verify(result.first == SUCCESS);
         verify(result.second != nullptr);
         Log_info("connected to client leader site: %d, %d, %p",
                  leader_site_info.id, leader_site_info.locale_id,
                  result.second);
         client_leaders_.push_back(
             std::make_pair(leader_site_info.id, result.second));
      }
   }
   client_leaders_connected_.store(true);
}
...
std::pair<siteid_t, ClassicProxy*> Communicator::LeaderProxyForPartition(
    parid_t par_id) const {
   auto leader_cache =
   ...
   if (leader_it != leader_cache.end()) {
      ...
      if (proxy_it == partition_proxies.end()) {
         ...
         Log_debug("leader site for parition %d is %d", par_id,
                   proxy_it->first);
      }
...
ClientSiteProxyPair Communicator::ConnectToClientSite(
    Config::SiteInfo& site, std::chrono::milliseconds timeout) {
   auto config = Config::GetConfig();
   char addr[1024];
...
std::pair<int, ClassicProxy*> Communicator::ConnectToSite(
    Config::SiteInfo& site, std::chrono::milliseconds timeout) {
...
void Communicator::BroadcastDispatch(
    uint32_t coo_id, shared_ptr<vector<shared_ptr<TxPieceData>>> sp_vec_piece,
    Coordinator* coo, const function<void(int, TxnOutput&)>& callback) {
   ...
   Log_debug("send dispatch to site %ld", pair_leader_proxy.first);
   auto proxy = pair_leader_proxy.second;
   shared_ptr<VecPieceData> sp_vpd(new VecPieceData);
   sp_vpd->sp_vec_piece_data_ = sp_vec_piece;
   MarshallDeputy md(sp_vpd);  // ????

   auto future = proxy->async_Dispatch(coo_id, cmd_id, md, fuattr);
   ...
   if (!broadcasting_to_leaders_only_) {
      for (auto& pair : rpc_par_proxies_[par_id]) {
         if (pair.first != pair_leader_proxy.first) {
            ...
            Future::safe_release(
                pair.second->async_Dispatch(coo_id, cmd_id, md, fu2));
         }
      }
   }
}

void Communicator::SendStart(SimpleCommand& cmd, int32_t output_size,
                             std::function<void(Future* fu)>& callback) {
   verify(0);
}

void Communicator::SendPrepare(groupid_t gid, txnid_t tid,
...
void Communicator::SendCommit(parid_t pid, txnid_t tid,
...
void Communicator::SendUpgradeEpoch(
    epoch_t curr_epoch,
    const function<void(parid_t, siteid_t, int32_t&)>& callback) {
...
void Communicator::SendForwardTxnRequest(
   ...
   Future::safe_release(
       leader_proxy->async_DispatchTxn(dispatch_request, future));
}

vector<shared_ptr<MessageEvent>> Communicator::BroadcastMessage(
    shardid_t shard_id, svrid_t svr_id, string& msg) {
   verify(0);
   // TODO
...
shared_ptr<MessageEvent> Communicator::SendMessage(siteid_t site_id,
                                                   string& msg) {
   verify(0);
   // TODO
...
void Communicator::AddMessageHandler(function<bool(const string&, string&)> f) {
