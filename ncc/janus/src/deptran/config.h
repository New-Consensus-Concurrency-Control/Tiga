...
class Config {
  ...
  public:
   ...
   std::string nezha_recorder_addr_;
   ...
   struct SiteInfo {
      ...
      uint32_t locale_id;  // represents a group of servers, such as those
      ...
      string GetRecordAddr() {
         ...
         if (host.empty()) host = proc_name;
         string ret;
         ret = ret.append(host).append(":").append(std::to_string(port + 5000));
         return ret;
      }
   };
