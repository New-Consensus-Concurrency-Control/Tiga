...
void client_launch_workers(vector<Config::SiteInfo>& client_sites) {
   ...
   Log_info("client enabled-3, number of sites: %d", client_sites.size());
   vector<ClientWorker*> workers;

   for (uint32_t client_id = 0; client_id < client_sites.size(); client_id++) {
      Log_info("Creating works %u", client_id);
      ClientWorker* worker =
          new ClientWorker(client_id, client_sites[client_id],
                           Config::GetConfig(), ccsi_g, nullptr);
      Log_info("Created works %u", client_id);
      ...
      Log_info("Fin works %u", client_id);
   }
   Log_info("client threads number=%lu", client_threads_g.size());
}
...
void server_launch_worker(vector<Config::SiteInfo>& server_sites) {
   ...
   for (auto& site_info : server_sites) {
      setup_ths.push_back(std::thread([&site_info, &i, &config]() {
         ...
         worker.site_info_ =
             const_cast<Config::SiteInfo*>(&config->SiteById(site_info.id));
         worker.SetupBase();
         ...
         worker.SetupService();
         Log_info("start communication for site %d",
                  (int)worker.site_info_->id);
         worker.SetupCommo();
...
void client_shutdown() { client_workers_g.clear(); }
