...
static int volatile x1 = MarshallDeputy::RegInitializer(
    MarshallDeputy::CMD_VEC_PIECE,
    []() -> Marshallable* { return new VecPieceData; });
...
set<parid_t>& TxData::GetPartitionIds() { return partition_ids_; }

bool TxData::IsOneRound() { return false; }

vector<TxPieceData> TxData::GetCmdsByPartitionAndRank(parid_t par_id,
                                                      rank_t rank) {
...
ReadyPiecesData TxData::GetReadyPiecesData(int32_t max) {
   ...
   for (auto& kv : status_) {
      ...
      // Log_info("piece id=%d status =%d", pi, status);
      if (status == DISPATCHABLE) {
         ...
         // Log_info("getting piece data piece id: %d", pi);
...
shared_ptr<TxPieceData> TxData::GetNextReadySubCmd() {
   ...
   for (auto& kv : status_) {
      ...
      if (status == DISPATCHABLE) {
         ...
         Log_debug("getting subcmd i: %d, thread id: %x", pi,
...
bool TxData::AllPiecesDispatchable() {
   return n_pieces_dispatchable_ == n_pieces_all_ &&
...
void TxRequest::get_log(i64 tid, std::string& log) {
   ...
   memcpy((void*)(log.data() + sizeof(tx_type_)), (void*)&input_size,
          sizeof(input_size));
