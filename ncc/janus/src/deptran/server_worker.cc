...
void ServerWorker::SetupHeartbeat() {
   ...
   std::string addr_port = std::string("0.0.0.0:") + std::to_string(port);
   ...
   Log_info("heartbeat setup for %s on %s", this->site_info_->name.c_str(),
            addr_port.c_str());
}
...
void ServerWorker::SetupBase() {
   ...
   tx_sched_->recorder_addr_ = site_info_->GetRecordAddr();
   ...
   if (rep_sched_ && tx_sched_) {
      rep_sched_->RegLearnerAction(
          std::bind(&TxLogServer::Next, tx_sched_, std::placeholders::_1));
   }
}
...
void ServerWorker::PopTable() {
   ...
   Log_info("data populated for site: %x, partition: %x", site_info_->id,
...
void ServerWorker::SetupService() {
   ...
   if (tx_frame_ != nullptr) {
      services_ = tx_frame_->CreateRpcServices(site_info_->id, tx_sched_,
                                               svr_poll_mgr_, scsi_);
   }

   if (rep_frame_ != nullptr) {
      auto s2 = rep_frame_->CreateRpcServices(site_info_->id, rep_sched_,
                                              svr_poll_mgr_, scsi_);
   ...
   Log_info("Server %s ready at %s", site_info_->name.c_str(),
            bind_addr.c_str());
}
...
void ServerWorker::WaitForShutdown() {
   ...
   if (hb_rpc_server_ != nullptr) {
      ...
      delete scsi_;
      if (svr_hb_poll_mgr_g != svr_poll_mgr_) svr_hb_poll_mgr_g->release();
      if (hb_thread_pool_g != svr_thread_pool_) hb_thread_pool_g->release();

      for (auto service : services_) {
         if (DepTranServiceImpl* s =
                 dynamic_cast<DepTranServiceImpl*>(service)) {
            auto& recorder = s->recorder_;
            if (recorder) {
               auto n_flush_avg_ = recorder->stat_cnt_.peek().avg_;
               auto sz_flush_avg_ = recorder->stat_sz_.peek().avg_;
               Log::info(
                   "Log to disk, average log per flush: %lld,"
