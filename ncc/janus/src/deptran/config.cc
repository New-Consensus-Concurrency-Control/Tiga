...
int Config::CreateConfig(int argc, char **argv) {
   ...
   verify(config_s == nullptr);
   config_s = new Config(ctrl_hostname, ctrl_port, ctrl_timeout, ctrl_key,
                         ctrl_init, tot_req_num, n_concurrent,
                         // ctrl_run,
                         duration, heart_beat, single_server, logging_path);
...
Config::Config(char *ctrl_hostname, uint32_t ctrl_port, uint32_t ctrl_timeout,
...
void Config::LoadYML(std::string &filename) {
   ...
   if (config["nezha-recorder"]) {
      nezha_recorder_addr_ = config["nezha-recorder"].as<std::string>();
   }
}
...
int Config::GetClientPort(std::string site_name) {
   ...
   for (auto s : sites) {
      if (std::find(hosts.begin(), hosts.end(), site_proc_map_[s]) ==
          hosts.end()) {
         if (s == site_name) {
...
void Config::LoadBenchYML(YAML::Node config) {
   ...
   if (config["coefficient"]) coeffcient_ = config["coefficient"].as<float>();
   if (config["rotate"]) rotate_ = config["rotate"].as<int32_t>();

   /* Dynamic workload parameters */
   if (config["txnsize"]) {
      txn_size_ = config["txnsize"].as<int32_t>();
      verify(n_dynamic_rows != 0 && txn_size_ > 0 &&
             txn_size_ <= n_dynamic_rows);
   }
...
void Config::GenerateFBTableColumnInfo(Sharding::tb_info_t &tbl_info) {
   ...
   int max_n_cols = 2;  // a hot optimization for saving memory, fb only
                        // populates 2 cols but will read/write multiple cols.
   for (int i = 0; i < max_n_cols; ++i) {
      ...
      std::string c_name =
          std::to_string(i);  // we use col_id also for col_name
      std::string c_foreign, ftbl_name, fcol_name;
      bool is_foreign = false;
      tbl_info.columns.push_back(Sharding::column_t(
          c_v_type, c_name, c_primary, is_foreign, ftbl_name, fcol_name));
   }
}
...
void Config::LoadSchemaTableColumnYML(Sharding::tb_info_t &tb_info,
   ...
   tb_info.columns.push_back(Sharding::column_t(
       c_v_type, c_name, c_primary, is_foreign, ftbl_name, fcol_name));
}
...
void Config::InitTPCCD() {
   ...
   if (benchmark_ == TPCC_REAL_DIST_PART) {
      i32 n_w_id = (i32)(tb_infos[std::string(TPCC_TB_WAREHOUSE)].num_records);
      verify(n_w_id > 0);
      i32 n_d_id =
          (i32)(GetNumPartition() *
      ...
      for (d_id = 0; d_id < n_d_id; d_id++)
         for (w_id = 0; w_id < n_w_id; w_id++) {
            MultiValue mv(std::vector<Value>({Value(d_id), Value(w_id)}));
            sharding_->insert_dist_mapping(mv);
         }
      i32 n_i_id = (i32)(tb_infos[std::string(TPCC_TB_ITEM)].num_records);
      i32 i_id = 0;

      for (i_id = 0; i_id < n_i_id; i_id++)
         for (w_id = 0; w_id < n_w_id; w_id++) {
            MultiValue mv(std::vector<Value>({Value(i_id), Value(w_id)}));
...
unsigned int Config::get_ctrl_port() { return ctrl_port_; }

unsigned int Config::get_ctrl_timeout() { return ctrl_timeout_; }

const char *Config::get_ctrl_hostname() { return ctrl_hostname_; }

const char *Config::get_ctrl_key() { return ctrl_key_; }

const char *Config::get_ctrl_init() { return ctrl_init_; }
...
std::vector<Config::SiteInfo> Config::SitesByPartitionId(parid_t partition_id) {
...
std::vector<Config::SiteInfo> Config::SitesByLocaleId(uint32_t locale_id,
                                                      SiteInfoType type) {
   std::vector<SiteInfo> result;
   std::vector<SiteInfo> *searching;
   if (type == SERVER) {
...
vector<Config::SiteInfo> Config::SitesByProcessName(string proc_name,
                                                    Config::SiteInfoType type) {
   std::vector<SiteInfo> result;
   std::vector<SiteInfo> *searching;
   if (type == SERVER) {
      searching = &sites_;
   ...
   std::for_each(searching->begin(), searching->end(),
                 [proc_name, &result](SiteInfo &site) mutable {
                    if (site.proc_name == "") {
                       Log_fatal("cannot find proc name for site %s",
                                 site.name.c_str());
                    }
...
unsigned int Config::get_duration() { return duration_; }

bool Config::do_heart_beat() { return heart_beat_; }

int Config::get_mode() { return tx_proto_; }
...
int Config::benchmark() { return benchmark_; }
...
unsigned int Config::get_scale_factor() { return scale_factor_; }

int32_t Config::get_max_retry() { return max_retry_; }

Config::single_server_t Config::get_single_server() { return single_server_; }

unsigned int Config::get_concurrent_txn() { return n_concurrent_; }

bool Config::get_batch_start() { return batch_start_; }

std::vector<double> &Config::get_txn_weight() { return txn_weight_; }

std::map<string, double> &Config::get_txn_weights() { return txn_weights_; };
...
bool Config::do_early_return() { return early_return_; }

bool Config::do_logging() { return logging_path_.empty(); }
...
const char *Config::log_path() { return logging_path_.c_str(); }

bool Config::retry_wait() { return retry_wait_; }

int32_t Config::get_tot_req() { return tot_req_num_; }
