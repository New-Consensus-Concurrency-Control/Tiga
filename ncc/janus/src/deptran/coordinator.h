...
class Coordinator {
  public:
   // Added by Jinkun
   uint64_t txn_start_time_;
   uint64_t txn_complete_time_;
   uint32_t txn_type_;
   ////
   ...
   Coordinator(uint32_t coo_id, int benchmark,
               ClientControlServiceImpl* ccsi = NULL, uint32_t thread_id = 0);

   virtual ~Coordinator();

   /** thread unsafe */
   uint64_t next_pie_id() { return this->next_pie_id_++; }
