...
class TxLogServer {
  public:
   ...
   rrr::PollMgr *nezha_poll_mgr_ = nullptr;
   PaxosRPC::PaxosProxy *nezha_proxy_ = nullptr;
   ...
   PaxosRPC::PaxosProxy *nezhaProxy() {
      verify(nezha_proxy_ != nullptr);
      return nezha_proxy_;
   }

   bool ConnectToNezhaRecoder();
   TxLogServer();
   TxLogServer(int mode);
   virtual ~TxLogServer();

   virtual void SetPartitionId(parid_t par_id) { partition_id_ = par_id; }

   // runs in a coroutine.

   virtual bool HandleConflicts(Tx &dtxn, innid_t inn_id,
   ...
   virtual void Execute(Tx &txn_box, innid_t inn_id);

   Coordinator *CreateRepCoord();
   virtual shared_ptr<Tx> GetTx(txnid_t tx_id);
   virtual shared_ptr<Tx> CreateTx(txnid_t tx_id, bool ro = false);
   virtual shared_ptr<Tx> CreateTx(epoch_t epoch, txnid_t txn_id,
   ...
   inline mdb::Table *get_table(const string &name) {
   ...
   void get_prepare_log(i64 txn_id, const std::vector<i32> &sids,
                        std::string *str);
   ...
   void reg_table(const string &name, mdb::Table *tbl);

   virtual bool Dispatch(cmdid_t cmd_id, shared_ptr<Marshallable> cmd,
