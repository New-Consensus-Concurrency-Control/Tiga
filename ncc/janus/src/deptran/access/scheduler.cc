...
int32_t SchedulerAcc::OnDispatch(cmdid_t cmd_id,
   ...
   */
   ...
   /* test code
   if (this->partition_id_ != coord) {
       Log_info("this svr = %d, sending to coord = %d, with cmd_id = %lu.",
   this->partition_id_, coord, cmd_id);
   ...
   if (this->partition_id_ == coord) {
       for (auto cohort : tx->record.cohorts) {
           Log_info("this svr = %d, is coord. sending to cohort = %d, with
   cmd_id = %lu.", this->partition_id_, cohort, cmd_id);
   ...
   */
   ...
   tx->load_speculative_ssid(
       ssid_spec);  // read in the spec ssid provided by the client
   if (!tx->cmd_) {
      tx->cmd_ = cmd;
   } else {
      auto present_cmd =
   ...
   Log_info("parid = %d; txnid = %lu; OnDispatch. coord = %u. cohorts = %s.",
   this->partition_id_, tx->tid_, coord, cs.c_str());
   */
   // get all the being-accessed row_keys, feed in predictor, and find decision
   uint64_t now =
       Predictor::get_current_time();  // the phyiscal arrival time of this tx
   *arrival_time = now;                // return arrival time in response
   // Log_info("server:OnDispatch. txid = %lu. ssid_spec = %lu.eturning
   // arrival_time = %lu.", tx->tid_, ssid_spec, *arrival_time);
   ...
   if (is_single_shard_write_only) {
      ...
      // Log_info("Mark all writes finalized due to the txn is write only and
      // single shard");
   }
   // now execute this txn
   // tx->acc_query_start->Set(1);  // acc_query RPC can be handled only after
   // we have executed this tx Log_debug("Received AccDispatch for txnid = %lu;
   // #pieces = %d.", tx->tid_, sp_vec_piece->size());
   for (const auto& sp_piece_data : *sp_vec_piece) {
      ...
      for (const auto& key : sp_piece_data->input.keys_) {
          Log_debug("piece input id = %d; row key = %d.", key,
      sp_piece_data->input.at(key).get_i32());
      }
      */
      if (sp_piece_data->root_type_ == FB_ROTXN ||
          sp_piece_data->root_type_ == SPANNER_ROTXN ||
          sp_piece_data->root_type_ == TPCC_STOCK_LEVEL) {
         tx->is_rotxn = true;
   ...
   // Log_info("parid = %d; txnid = %lu; indices size = %d.",
   // this->partition_id_, tx->tid_, tx->sg.metadata.indices.size()); Step 2:
   // report ssid status
   ...
   *ssid_new = tx->sg.metadata.highest_write_ssid;
   ...
   // rotxn
   *rotxn_okay = 1;
   // if (sp_vec_piece->at(0)->root_type_ == FB_ROTXN ||
   // sp_vec_piece->at(0)->root_type_ == SPANNER_ROTXN ||
   // sp_vec_piece->at(0)->root_type_ == TPCC_STOCK_LEVEL) {
...
void SchedulerAcc::OnValidate(cmdid_t cmd_id, snapshotid_t ssid_new,
                              int8_t* res) {
   auto acc_txn =
       dynamic_pointer_cast<AccTxn>(GetOrCreateTx(cmd_id));  // get the txn
   // Log_info("parid = %d; txnid = %lu; Received validation.",
   // this->partition_id_, acc_txn->tid_);
   /// Log_info("parid = %d; txnid = %lu; Validation. status_before = %d,
   /// ssid_low = %lu, ssid_high = %lu.", this->partition_id_, acc_txn->tid_,
   /// acc_txn->record.status, acc_txn->record.ts_low, acc_txn->record.ts_high);
   if (acc_txn->sg.validate_done) {
      verify(0);  // should never get here
      // multiple pieces may share the same scheduler and thus validate on the
      // same indices map
      *res = CONSISTENT;  // if there is at least one validation fails, final
                          // result will be fail
      return;
   }
   // TODO: here we do async lambda callback that allows reads to wait for
   // dependent versions to finalize
   ...
   for (auto& row_col : acc_txn->sg.metadata.indices) {
      ...
      for (auto& col_ssid : row_col.second) {
         if (!acc_row->validate(acc_txn->tid_, col_ssid.first, col_ssid.second,
                                ssid_new, validate_consistent)) {
   ...
   /// Log_info("parid = %d; txnid = %lu; Validation. status_after = %d,
   /// ssid_low = %lu, ssid_high = %lu.", this->partition_id_, acc_txn->tid_,
   /// acc_txn->record.status, acc_txn->record.ts_low, acc_txn->record.ts_high);
...
void SchedulerAcc::OnFinalize(cmdid_t cmd_id, int8_t decision) {
   auto acc_txn =
       dynamic_pointer_cast<AccTxn>(GetOrCreateTx(cmd_id));  // get the txn
   /// Log_info("parid = %d; txnid = %lu; Finalize. status_before = %d, ssid_low
   /// = %lu, ssid_high = %lu.", this->partition_id_, acc_txn->tid_,
   /// acc_txn->record.status, acc_txn->record.ts_low, acc_txn->record.ts_high);
   // failure handling
   acc_txn->record.status =
       decision;  // set status: either FINALIZED OR ABORTED
   /// Log_info("parid = %d; txnid = %lu; Finalize. status_after = %d, ssid_low
   /// = %lu, ssid_high = %lu.", this->partition_id_, acc_txn->tid_,
   /// acc_txn->record.status, acc_txn->record.ts_low, acc_txn->record.ts_high);
   // Log_info("parid = %d; txnid = %lu; Finalizing RPC. decision = %d.",
   // this->partition_id_, acc_txn->tid_, decision);
   if (acc_txn->sg.metadata.indices.empty()) {
      // we've done finalize for this txn already
      // Log_info("txnid = %lu; Finalizing RPC. decision = %d. INDICES EMPTY,
      // returned.", acc_txn->tid_, decision); verify(0); // should not get here
      return;
   }
   ...
   for (auto& row_col : acc_txn->sg.metadata.indices) {
      ...
      for (auto& col_index : row_col.second) {
         acc_row->finalize(acc_txn->tid_, col_index.first, col_index.second,
                           decision);
      }
   }
   /*
...
void SchedulerAcc::OnStatusQuery(cmdid_t cmd_id, int8_t* res,
                                 DeferredReply* defer) {
   verify(0);
   auto acc_txn =
       dynamic_pointer_cast<AccTxn>(GetOrCreateTx(cmd_id));  // get the txn
   ...
   /// Log_info("parid = %d; txnid = %lu; StatusQuery. status_before = %d,
   /// ssid_low = %lu, ssid_high = %lu.", this->partition_id_, acc_txn->tid_,
   /// acc_txn->record.status, acc_txn->record.ts_low, acc_txn->record.ts_high);
   ...
   // do a round of check in case there is validation or finalize arrive in
   // between
   ...
   // now we check each pending version, and insert a callback func to that
   // version waiting for its status
   ...
   for (auto& row_col : acc_txn->sg.metadata.reads_for_query) {
      ...
      for (auto& col_index : row_col.second) {
         ...
         txnid_t to_tid = acc_row->get_ver_tid(
             col_index.first,
             col_index
                 .second);  // inserting the callback of current txn to txn:tid
         auto to_txn = dynamic_pointer_cast<AccTxn>(
             GetOrCreateTx(to_tid));  // get the target txn
   ...
   for (auto& row_col : acc_txn->sg.metadata.writes_for_query) {
      ...
      for (auto& col_index : row_col.second) {
         ...
         // acc_txn->n_query_inc();
         acc_txn->subrpc_count[rpc_id]++;
         // txnid_t to_tid = acc_row->get_ver_tid(col_index.first,
         // col_index.second);  // inserting the callback of current txn to
         // txn:tid auto to_txn =
         // dynamic_pointer_cast<AccTxn>(GetOrCreateTx(to_tid));  // get the
         // target txn
         acc_row->insert_write_callbacks(col_index.first, col_index.second,
                                         acc_txn, res, defer, rpc_id);
         // to_txn->insert_write_callbacks(acc_txn, res, defer, rpc_id);
      }
   }
   acc_txn->sg.metadata.reads_for_query
       .clear();  // no need those records anymore, might add more by later
                  // dispatch
   acc_txn->sg.metadata.writes_for_query.clear();
}

void SchedulerAcc::check_status(cmdid_t cmd_id, bool& is_decided,
                                bool& will_abort) {
   verify(0);
   auto acc_txn =
       dynamic_pointer_cast<AccTxn>(GetOrCreateTx(cmd_id));  // get the txn
   for (auto& row_col : acc_txn->sg.metadata.reads_for_query) {
      ...
      for (auto& col_index : row_col.second) {
         int8_t status =
             acc_row->check_status(col_index.first, col_index.second);
         switch (status) {
            case UNCHECKED:
            case VALIDATING:
               is_decided = false;
   ...
   for (auto& row_col : acc_txn->sg.metadata.writes_for_query) {
      auto acc_row = dynamic_cast<AccRow*>(row_col.first);
      for (auto& col_index : row_col.second) {
         bool write_safe = acc_row->check_write_status(
             acc_txn->tid_, col_index.first, col_index.second);
         if (!write_safe) {
            is_decided = false;
         } else {
            col_index.second = -1;
...
i32 SchedulerAcc::get_row_key(const shared_ptr<SimpleCommand>& sp_piece_data,
                              int32_t var_id, uint8_t workload) const {
   switch (workload) {
      case FACEBOOK:
      ...
      case TPCC: {
         ...
         if (sp_piece_data->input.values_->find(var_id) ==
             sp_piece_data->input.values_->end()) {
            // key/value dependent, not ready yet
            return NOT_ROW_KEY;
         }
         Value k = sp_piece_data->input.at(var_id);
         if (k.get_kind() != mdb::Value::I32) {
            // tpcc, either a string: by_last (not really readcolumn or
            // writecolumn) or unfilled key or a double used as col values not
            // row identifiers
            return NOT_ROW_KEY;
         } else {
            return sp_piece_data->input.at(var_id).get_i32();
...
bool SchedulerAcc::tpcc_row_key(int32_t var_id) const {
   return (var_id == TPCC_VAR_C_ID || var_id == TPCC_VAR_D_ID ||
           var_id == TPCC_VAR_W_ID || var_id == TPCC_VAR_O_ID ||
           var_id == TPCC_VAR_C_D_ID || var_id == TPCC_VAR_C_W_ID ||
...
void SchedulerAcc::OnResolveStatusCoord(cmdid_t cmd_id, uint8_t* status,
   ...
   // Log_info("COORD. OnResolveStatusCoord this svr = %d, receiving cmd_id =
   // %lu.", this->partition_id_, cmd_id);
   ...
   // resolve_status needs to query cohorts to reconstruct status, we delay
   // return
   auto tx = dynamic_pointer_cast<AccTxn>(GetOrCreateTx(cmd_id));
...
void SchedulerAcc::AccResolveStatusCoordAck(cmdid_t tid, uint8_t status) {
   // Log_info("AccResolveStatusCoordAck returned tid = %lu; status = %d.", tid,
   // status); Log_info("Cohort. AccResolveStatusCoordAck this svr = %d,
   // receiving cmd_id = %lu, status = %d.", this->partition_id_, tid, status);
...
void SchedulerAcc::OnGetRecord(uint64_t cmd_id, uint8_t* status,
                               uint64_t* ssid_low, uint64_t* ssid_high) {
   // Log_info("Cohort. OnGetRecord. this svr = %d, receiving cmd_id = %lu.",
   // this->partition_id_, cmd_id);
   auto tx = dynamic_pointer_cast<AccTxn>(GetOrCreateTx(cmd_id));
...
void SchedulerAcc::AccGetRecordAck(uint64_t tid, uint8_t status,
                                   uint64_t ssid_low, uint64_t ssid_high) {
   // Log_info("COORD. AccGetRecordAck. this svr = %d, receiving cmd_id = %lu,
   // status = %d, ssid_low = %lu, ssid_high = %lu.",
   ...
   if (tx->get_record_acks == tx->get_record_rpcs) {
      ...
      if (tx->returned_records.find(FINALIZED) != tx->returned_records.end()) {
         decision = FINALIZED;
      } else if (tx->returned_records.find(ABORTED) !=
                 tx->returned_records.end()) {
         decision = ABORTED;
      } else if (tx->returned_records.find(UNCLEARED) !=
                 tx->returned_records.end()) {
         decision = UNCLEARED;
      }
...
txn_status_t SchedulerAcc::resolve_status(uint64_t cmd_id) {
   ...
   for (auto cohort : tx->record.cohorts) {
      tx->get_record_rpcs++;
      commo()->AccBroadcastGetRecord(
          cohort, cmd_id,
          std::bind(&SchedulerAcc::AccGetRecordAck, this, cmd_id,
...
void SchedulerAcc::handle_failure(uint64_t cmd_id, uint32_t pid) {
   ...
   // auto tx = dynamic_pointer_cast<AccTxn>(GetOrCreateTx(cmd_id));
   // Log_info("handle_failure triggered. this svr = %d, for cmd_id = %lu.",
   // pid, cmd_id);
   if (tx->record.status != CLEARED) {
      // Log_info("handle_failure triggered early return. this svr = %d, for
      // cmd_id = %lu. current_status = %d. handle_failure = %d.",
      //          pid, cmd_id, tx->record.status, tx->handle_failure);
      return;
   }
   if (tx->record.coord == pid) {  // this is coord
      // Log_info("coord. handle_failure. this svr = %d, for cmd_id = %lu.",
      // pid, cmd_id);
      resolve_status(cmd_id);
   } else {  // this is cohort
      // Log_info("cohort. handle_failure. this svr = %d, for cmd_id = %lu.",
      // pid, cmd_id);
      auto coord = tx->record.coord;
      commo()->AccBroadcastResolveStatusCoord(
          coord, cmd_id,
          std::bind(&SchedulerAcc::AccResolveStatusCoordAck, this, cmd_id,
                    std::placeholders::_1));
   }
}
...
i32 SchedulerAcc::get_key(const shared_ptr<SimpleCommand>& c) const {
   //                Log_info("Workload type = %d, op count = %d, keys_ size =
   //                %d. keys_ real size = %d. keys_ = %d. values size = %d. key
   //                = %d.",
   //                         c->root_type_, c->input.at(FB_OP_COUNT).get_i32(),
   //                         c->input.size(), c->input.keys_.size(),
   //                         *(c->input.keys_.begin()),
   //                         c->input.values_->size(),
   //                         c->input.at(*(c->input.keys_.begin())).get_i32());
   //
   int workload = c->root_type_;
   // TODO: we only implement for facebook and spanner for now. TPCC comes
   // later?
   switch (workload) {
      case FB_ROTXN: {  //  this is a rotxn in FB, FB input.keys_ only store 1
                        //  key per command
         int key_index = *(c->input.keys_.begin());
         return c->input.at(key_index).get_i32();
      }
      case SPANNER_ROTXN: {  // this is a rotxn in Spanner, Spanner keys_ store
                             // 1 key, a SPANNER_TXN_SIZE, and a
                             // SPANNER_RW_W_COUNT
         int key_index = *(c->input.keys_.begin());
         return c->input.at(key_index).get_i32();
      }
