...
void CoordinatorAcc::GotoNextPhase() {
   ...
   switch (phase_++ % n_phase) {
      case Phase::INIT_END:
         verify(phase_ % n_phase == Phase::SAFEGUARD);
         unfreeze_coordinator();
         DispatchAsync();
         break;
      case Phase::SAFEGUARD:
         verify(phase_ % n_phase == Phase::FINISH);
         SafeGuardCheck();
         break;
      case Phase::FINISH:
...
void CoordinatorAcc::DispatchAsync() {
   ...
   // auto cmds_by_par = txn->GetReadyPiecesData(pieces_per_hop); // all ready
   // pieces sent in one parallel round Log_info("AccDispatchAsync for tx_id: %"
   // PRIx64, txn->root_id_); check if this txn is a single-shard txn: if so,
   // always consistent and no need to validate
   ...
   */
   ...
   // TODO: use dispatched == 0 instead of ssid_spec == 0 to start the following
   // block!
   if (tx_data().ssid_spec == 0) {
      // for multi-shot txns, we only do ssid prediction at the first shot and
      // use the same ssid across shots
   ...
   // Log_info("Client:Dispatch. txid = %lu. current_time = %lu. sending
   // ssid_spec = %lu.", txn->id_, current_time, tx_data().ssid_spec);
   for (auto& pair : cmds_by_par) {
      ...
      // Log_info("partition id =%d  pieces =%d", par_id, cmds.size());
      for (const auto& c : cmds) {
         /*
         Log_info("txid = %lu. Workload type = %d, op count = %d, keys_ size =
         %d. keys_ real size = %d. keys_ = %d. values size = %d. key = %d.",
                  txn->id_, c->root_type_, c->input.at(FB_OP_COUNT).get_i32(),
         c->input.size(), c->input.keys_.size(), *(c->input.keys_.begin()),
                  c->input.values_->size(),
         c->input.at(*(c->input.keys_.begin())).get_i32());
         */
         // Log_info("try this. roottype = %d; type = %d.", c->root_type_,
         // c->type_);
         //  for rotxn
         if (c->root_type_ == FB_ROTXN || c->root_type_ == SPANNER_ROTXN ||
             c->root_type_ == TPCC_STOCK_LEVEL) {
            tx_data().is_rotxn = true;
            // i32 key = get_key(c);
            // i64 ts = get_ts(key);
            // c->input[TS_INDEX] = Value(ts);
         }
         // Log_info("workload type = %d. txn_id = %lu. key = %d.",
         // c->root_type_, txn->id_, key);
         tx_data().innid_to_server[c->inn_id_] = par_id;
         tx_data().innid_to_starttime[c->inn_id_] = current_time;
         // Log_info("Client:Dispatch. txid = %lu. recording innid = %u. server
         // = %u.", txn->id_, c->inn_id_, par_id);
         c->id_ = next_pie_id();
         c->op_type_ = txn_reg_->get_optype(c->root_type_, c->type_);
         if (!txn_reg_->get_write_only(c->root_type_, c->type_)) {
            write_only = false;  // write-only if all pieces for this parid are
                                 // write-only pieces
         }
      ...
      */
      if (tx_data().is_rotxn) {  // rotxn
         commo()->AccBroadcastRotxnDispatch(
             this->coo_id_, sp_vec_piece, this, tx_data().ssid_spec,
             get_ts(par_id),
             std::bind(&CoordinatorAcc::AccDispatchAck, this, phase_,
                       std::placeholders::_1, std::placeholders::_2,
                       std::placeholders::_3, std::placeholders::_4,
                       std::placeholders::_5, std::placeholders::_6,
                       std::placeholders::_7, std::placeholders::_8));
      } else {  // read-write txn
         commo()->AccBroadcastDispatch(
             this->coo_id_, sp_vec_piece, this, tx_data().ssid_spec,
   ...
   //  Log_info("AccDispatchAsync cnt: %d for tx_id: %" PRIx64, cnt,
   //  txn->root_id_);
}

void CoordinatorAcc::AccDispatchAck(
    phase_t phase, int res, uint64_t ssid_low, uint64_t ssid_high,
    uint64_t ssid_new, map<innid_t, map<int32_t, Value>>& outputs,
   ...
   // Log_info("rotxn_okay = %d; returned_ts size = %d, returned_ts[-1] = %lu.",
   // rotxn_okay, returned_ts.size(), returned_ts.at(-1)); for rotxn update
   // key_ts
   /*
   ...
   */
   ...
   for (auto& pair : outputs) {
      ...
      //(tx_data().innid_to_server.find(inn_id) !=
      // tx_data().innid_to_server.end());
      // verify(tx_data().innid_to_starttime.find(inn_id) !=
      // tx_data().innid_to_starttime.end());
      parid_t server = tx_data().innid_to_server[inn_id];
      uint64_t starttime = tx_data().innid_to_starttime[inn_id];
      uint64_t time_delta = arrival_time >= starttime
                                ? arrival_time - starttime
                                : 0;  // for clock skew (rare case)
      // Log_info("Client:ACK. txid = %lu. innid = %lu. server = %lu. time_delta
      // = %lu.", txn->id_, inn_id, server, time_delta);
   ...
   if (txn->HasMoreUnsentPiece()) {
      //   Log_info(
      //       "command has more sub-cmd, cmd_id: %llx,"
...
void CoordinatorAcc::AccStatusQueryAck(txnid_t tid, int8_t res) {
    ...
    if (is_frozen()) return;  // this txn has been decided, either committed or
aborted if (tx_data()._status_query_done) return; // no need to wait on query
acks, it has been decided tx_data().n_status_callback++;
    ...
    if (tx_data().n_status_query == tx_data().n_status_callback) { // received
all callbacks tx_data()._status_query_done = true; if (tx_data()._status_abort)
{ aborted_ = true;
        }
        committed_ = !aborted_;
        // Log_info("tx: %lu, time as usual. commit = %d", tx_data().id_,
committed_); if (phase_ % n_phase == Phase::INIT_END) { // this txn is waiting
in FINAL phase
            // Log_info("tx: %lu, time goes back. commit = %d", tx_data().id_,
committed_); time_flies_back();
        }
        // this txn has not got to FINAL phase yet, either validating or
dispatching
    }
}
*/
...
void CoordinatorAcc::SafeGuardCheck() {
   ...
   */
   ...
   if (tx_data()._early_abort ||
       (tx_data().is_rotxn && !tx_data().is_rotxn_okay)) {
      committed_ = false;
      aborted_ = true;
      GotoNextPhase();
      return;
   ...
   if (committed_ || aborted_) {  // SG checks consistent or cascading aborts,
                                  // no need to validate
      // Log_info("tx: %lu, safeguard check, commit = %d; aborted = %d",
      // tx_data().id_, committed_, aborted_);
      GotoNextPhase();
   } else {
      AccValidate();
   }
...
bool CoordinatorAcc::offset_1_check_pass() {
    ...
    return !tx_data()._offset_invalid &&  // write valid
           tx_data().highest_ssid_low - tx_data().lowest_ssid_high <= 1;  //
offset within 1
}
*/
...
void CoordinatorAcc::AccValidate() {
   ...
   for (auto par_id : pars) {
      ...
      commo()->AccBroadcastValidate(
          par_id, tx_data().id_,
          tx_data().ssid_new,  // the new cross-shard ssid for writes
          std::bind(&CoordinatorAcc::AccValidateAck, this, phase_,
                    std::placeholders::_1));
   }
}
...
void CoordinatorAcc::AccValidateAck(phase_t phase, int8_t res) {
   ...
   if (tx_data().n_validate_ack_ == tx_data().n_validate_rpc_) {
      ...
      // Log_info("tx: %lu, acc_validate_ack. commit = %d", tx_data().id_,
      // committed_);
...
void CoordinatorAcc::AccCommit() {
   ...
   if (tx_data()._decided || tx_data()._status_query_done) {
      ...
      if (!tx_data().pars_to_finalize.empty() ||
          tx_data()._is_consistent) {  // for ss, if didn't do validation, we
                                       // need to finalize reads
...
void CoordinatorAcc::AccAbort() {
   ...
   //  (aborted_);
   freeze_coordinator();
   // abort as long as inconsistent or there is at least one statusquery ack is
   // abort (cascading abort) for stats
   if (tx_data()._early_abort) {
      // tx_data().n_early_aborts++;
      n_early_aborts++;
   }
   if (!tx_data()._early_abort &&
       (tx_data()._is_consistent || !tx_data()._validation_failed)) {
...
void CoordinatorAcc::AccFinalize(int8_t decision) {
   ...
   std::lock_guard<std::recursive_mutex> lock(mtx_);
   // Log_info("tx: %lu, sending finalizeRPC. decision = %d.", tx_data().id_,
   // decision); auto pars = tx_data().GetPartitionIds();

   /* for testing failure mode
   if (tx_data().id_ % 1000 == 438) {
       // Log_info("Client injects failure on tx: %lu. decision = %d.",
   tx_data().id_, decision); return;
   }
   */
   if (tx_data().is_rotxn) {
      // now rotxns do not finalize
      verify(!tx_data()._early_abort);
      if (tx_data()._is_consistent && !tx_data()._status_abort &&
          decision == ABORTED) {
         n_rotxn_aborts++;
      }
      if (decision == ABORTED) {
   ...
   switch (decision) {
      case FINALIZED:
         for (auto par_id : pars) {
            commo()->AccBroadcastFinalize(par_id, tx_data().id_, decision);
         }
         break;
      case ABORTED:
         for (auto par_id : pars) {
            tx_data().n_abort_sent++;
            commo()->AccBroadcastFinalizeAbort(
                par_id, tx_data().id_, decision,
                std::bind(&CoordinatorAcc::AccFinalizeAck, this, phase_));
         }
         break;
      default:
         verify(0);
...
i32 CoordinatorAcc::get_key(const shared_ptr<SimpleCommand>& c) {
    std::lock_guard<std::recursive_mutex> lock(this->mtx_);
//                Log_info("Workload type = %d, op count = %d, keys_ size = %d.
keys_ real size = %d. keys_ = %d. values size = %d. key = %d.",
...
later? switch(workload) { case FB_ROTXN: { //  this is a rotxn in FB, FB
input.keys_ only store 1 key per command int key_index =
*(c->input.keys_.begin()); return c->input.at(key_index).get_i32();
        }
        case SPANNER_ROTXN: { // this is a rotxn in Spanner, Spanner keys_ store
1 key, a SPANNER_TXN_SIZE, and a SPANNER_RW_W_COUNT int key_index =
*(c->input.keys_.begin()); return c->input.at(key_index).get_i32();
        }
        default:
            return -1;
