...
shared_ptr<Tx> TxLogServer::CreateTx(epoch_t epoch, txnid_t tid,
                                     bool read_only) {
...
void TxLogServer::get_prepare_log(i64 txn_id, const std::vector<i32> &sids,
                                  std::string *str) {
   ...
   memcpy((void *)(str->data() + len), (void *)&prepare_tag,
          sizeof(prepare_tag));
   ...
   memcpy((void *)(str->data() + len), (void *)&num_servers,
          sizeof(num_servers));
...
Coordinator *TxLogServer::CreateRepCoord() {
   ...
   coord = rep_frame_->CreateCoordinator(cid++, Config::GetConfig(), benchmark,
                                         nullptr, id++, txn_reg_);
 ...
 */
void TxLogServer::Execute(Tx &txn_box, innid_t inn_id) {
   if (inn_id == 0) {
...
void TxLogServer::reg_table(const std::string &name, mdb::Table *tbl) {
...
void TxLogServer::TriggerUpgradeEpoch() {
   if (site_id_ == 0) {
      ...
      commo()->SendUpgradeEpoch(
          epoch,
          std::bind(&TxLogServer::UpgradeEpochAck, this, std::placeholders::_1,
                    std::placeholders::_2, std::placeholders::_3));
   }
}

void TxLogServer::UpgradeEpochAck(parid_t par_id, siteid_t site_id,
                                  int32_t res) {
...
bool TxLogServer::ConnectToNezhaRecoder() {
   if (nezha_proxy_) {
      Log_info("nezha proxy already created ");
      return false;
   }
   if (recorder_addr_ == "") {
      Log_info("recorder addr not set ");
      return false;
   }
   nezha_poll_mgr_ = new PollMgr(1);
   auto nezha_client = std::make_shared<rrr::Client>(nezha_poll_mgr_);

   int ret = -1;
   do {
      Log_info("Connecting to Nezha addr %s", recorder_addr_.c_str());
      ret = nezha_client->connect(recorder_addr_.c_str());
      Log_info("1-Connecting to Nezha addr %s", recorder_addr_.c_str());
      if (ret == 0) {
         nezha_proxy_ = new PaxosRPC::PaxosProxy(nezha_client.get());
         Log_info("Connected to Nezha addr %s", recorder_addr_.c_str());
         break;
      } else {
         sleep(1);
      }
