...
static uint64_t GetMicrosecondTimestamp(const int64_t offset = 0) {
...
class Communicator {
 public:
  const int CONNECT_TIMEOUT_MS = 1200*1000;
  ...
  void BroadcastDispatch(uint32_t coo_id, shared_ptr<vector<shared_ptr<SimpleCommand>>> vec_piece_data,
                         Coordinator *coo,
