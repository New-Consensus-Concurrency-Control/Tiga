...
if __name__ == '__main__':
    ...
    for s in range(num_shards):
        for r in range(num_replicas):
            server_names.append(tiga_common.TAG
                +"-server-s-"+str(s).zfill(2)+"-r-"+str(r).zfill(2))

    proxy_names = [ tiga_common.TAG +"-proxy-"+str(i).zfill(4) 
    ...
    for region in tiga_common.SERVER_REGIONS:
        ...
        gcp_tools.start_instance_list(instances_to_start, region)
