...
if __name__ == '__main__':
    ...
    for s in range(num_shards):
        for r in range(num_replicas):
            server_name = TAG+"-server-s-"+str(s).zfill(2)+"-r-"+str(r).zfill(2)
            create_instance(instance_name = server_name,
                            image= IMAGE_FAMILY,
    ...
    # for r in range(num_replicas):
    #     replica_name = TAG+"-replica-s-00"+"-r-"+str(r).zfill(2)
    #     create_instance(instance_name = replica_name,
    #                     image= IMAGE_FAMILY,
    #                     machine_type =  server_vm_type,
    #                     customzedZone=server_regions[r])
    #     print(colored("Created "+replica_name, "green", attrs=['bold']))
